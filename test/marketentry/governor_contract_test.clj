  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Hodan Trading Company"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Hodan Trading Company" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest beneficial-owner-registration-noncompliant-is-held-and-unoverridable
  (testing "declared beneficial-owner-pct above threshold but not disclosed within deadline -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:beneficial-owner-registration-noncompliant} (-> (store/ledger db) last :basis)))
...
(deftest tcc-record-unverified-is-held-and-unoverridable
  (testing "unverified Tax Compliance Certificate record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tcc-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
