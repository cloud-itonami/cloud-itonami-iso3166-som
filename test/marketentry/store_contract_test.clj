...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Trading Company" :jurisdiction "SOM"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :beneficial-owner-pct 35 :beneficial-owner-control? false
                                   :beneficial-owner-disclosed-within-deadline? true :beneficial-owner-noncompliant-days 0
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "SOM" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:beneficial-owner-pct (:engagement m)) (:beneficial-owner-pct (:engagement d))))
