...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (SOM, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up beneficial-owner-registration-noncompliant) ==")
    ...
    (println "== filing/submit eng-4 (beneficial-owner-registration-noncompliant -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tcc-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tcc-record-unverified -> HARD hold) ==")
